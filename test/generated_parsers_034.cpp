...
TEST(parser, generated_034_000)
{
    ...
    }
    {
        ...
        EXPECT_FALSE(fail_attr);
        {
            tuple<std::vector<char>, variant<std::vector<char>, optional<int>>, std::vector<int>> attr;
            auto const copy = attr;
            EXPECT_FALSE(parse(first, last, fail_parser, attr));
            EXPECT_EQ(attr, copy);
        }
