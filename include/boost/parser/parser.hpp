...
#if 0
            BOOST_PARSER_INSTRUMENT_CASE("STRING COMPARE BEGIN");
            std::pair<Iter1, Iter2> retval{first1, first2};
            while (retval.first != last1 && retval.second != last2) {
                if ((common_t)*retval.first == (common_t)*retval.second) {
                    ++retval.first;
                    ++retval.second;
                } else if (!no_case) {
                    break;
                } else {
                    case_fold_array_t folded1;
                    auto const folded1_last =
                        detail::case_fold(*retval.first, folded1.begin());
                    case_fold_array_t folded2;
                    auto const folded2_last =
                        detail::case_fold(*retval.second, folded2.begin());
                    ...
                    if (mismatch.first == folded1_last &&
                        mismatch.second == folded2_last) {
                        BOOST_PARSER_INSTRUMENT_CASE(1);
                        ++retval.first;
                        ++retval.second;
                    } else if (
                        mismatch.first != folded1_last &&
                        mismatch.second != folded2_last) {
                        BOOST_PARSER_INSTRUMENT_CASE(2);
                        break;
                    } else if (mismatch.first == folded1_last) {
                        auto const other_mismatch =
                            detail::no_case_aware_string_mismatch(
                                std::next(retval.first),
                                last1,
                                mismatch.second,
                                folded2_last,
                                no_case);
                        if (other_mismatch.second == folded2_last) {
                            BOOST_PARSER_INSTRUMENT_CASE(3);
                            retval.first = other_mismatch.first;
                            ++retval.second;
                        } else {
                            BOOST_PARSER_INSTRUMENT_CASE(4);
                            break;
                        }
                    } else { // implies mismatch.second == folded2_last
                        auto const other_mismatch =
                            detail::no_case_aware_string_mismatch(
                                std::next(retval.second),
                                last2,
                                mismatch.first,
                                folded1_last,
                                no_case);
                        if (other_mismatch.second == folded1_last) {
                            BOOST_PARSER_INSTRUMENT_CASE(5);
                            retval.second = other_mismatch.first;
                            ++retval.first;
                        } else {
                            BOOST_PARSER_INSTRUMENT_CASE(6);
                            break;
                        }
                    }
                }
            }
            BOOST_PARSER_INSTRUMENT_CASE("STRING COMPARE END\n\n");
            return retval;
#endif
        }
