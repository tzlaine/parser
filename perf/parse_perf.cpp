  ...
  - Atlanta Braves
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/basic_yaml_block.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  - c
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/basic_yaml_block2.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  - c
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/basic_yaml_block3.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
[
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/basic_yaml_flow.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
- Last)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/block_literals.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  0.278 Batting Average
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/block_literals2.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
h: Yes we have No bananas  # a string, "Yes" and "No" disambiguated by context.)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/data-types.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
...)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/relational.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  - b)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_array5.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  b: 2)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_array6.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  avg:  0.288)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_array7.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
Lemmon : !!map {a:1, b:2, c:3})YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map1.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
: 9)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map10.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
z: zz)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map11.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  - Atlanta Braves)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map12.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
Foo : on the right # strict yaml 1.2 disallows "on" as a boolean value!)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map13.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  y : 3)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map2.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  y : 3)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map5.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  key2: more value
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map6.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
Orange :)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map7.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
: 4)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map8.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  b: 2)YAML_TEXT";
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/yaml_block_map9.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
d: e
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/simple_flow_plain.yml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
...
--- text
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/escaped_percent_27NA.yml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
  ...
  - Atlanta Braves
    ...
    while (state.KeepRunning()) {
        result = parse_yaml(
            g_parser,
            contents, contents + strlen(contents),
            "../test/test_files/basic_yaml_block_repeated.yaml",
            reporting_fn_t(), reporting_fn_t()
        );
    }
    for (auto const & v : *result) {
