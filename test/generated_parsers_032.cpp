...
TEST(parser, generated_032_018)
{
    ...
    }
    {
        ...
        EXPECT_FALSE(fail_attr);
        {
            tuple<std::vector<char>, variant<optional<int>, std::vector<char>>, std::vector<int>> attr;
            auto const copy = attr;
            EXPECT_FALSE(parse(first, last, fail_parser, attr));
            EXPECT_EQ(attr, copy);
        }
